...
namespace UnityVolumeRendering
{
    public class VolumeRendererEditorFunctions
    {
        ...
        [MenuItem("Volume Rendering/Load dataset/Load v3draw dataset")]
        static void ShowV3dRawImporter()
        {
            string file = EditorUtility.OpenFilePanel("Select a dataset to load", "DataFiles", "");
            if (File.Exists(file))
            {
                V3DRawDatasetImporterEditorWindow wnd = (V3DRawDatasetImporterEditorWindow)EditorWindow.GetWindow(typeof(V3DRawDatasetImporterEditorWindow));
                if (wnd != null)
                    wnd.Close();

                wnd = new V3DRawDatasetImporterEditorWindow(file);
                wnd.Show();
            }
            else
            {
